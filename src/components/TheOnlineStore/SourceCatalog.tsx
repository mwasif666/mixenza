...
interface Props { products: SourceProduct[] }
const PAGE_SIZE = 24
...
const SourceCatalog: React.FC<Props> = ({ products }) => {
    ...
    const filtered = useMemo(() => {
        ...
        const result = products.filter(product => {
            ...
            const queryMatch = !normalized || [product.name, product.category, ...product.tags].some(value => value.toLowerCase().includes(normalized))
        ...
        <section className="container py-14 md:py-20">
            <div className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <h2 className="heading3 mt-2">Shop the collection</h2>
                    <p className="mt-2 text-secondary">{filtered.length} products · authentic catalog synced from TheOnlineStore</p>
                </div>
